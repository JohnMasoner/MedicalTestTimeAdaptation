...
class BaseTrainModel:
    ...
    def _build_parameters(self):
        ...
        self.total_epoch = 1 if self.cfg.env.smoke_test else self.cfg.slover.total_epoch
        ...
        self.is_distributed_train = (
            self.cfg.env.is_distributed_train
            if self.is_cuda and self.world_size > 1
            else False
        )

    def _build_writes(self):
        self.base_save_dir = os.path.join(
            self.cfg.env.save_dir,
        ...
        if self.is_main_process:
            ...
            if self.cfg.data_loader.semi_dataset:
                shutil.copy(
                    "./semi_config.yaml", self.code_save_dir + "/semi_config.yaml"
                )
            else:
                shutil.copy(
                    "./full_config.yaml", self.code_save_dir + "/full_config.yaml"
                )
    ...
    def _build_dataset(self):
        ...
        self.num_worker = (
            self.cfg.data_loader.batch_size + 2
            if self.cfg.data_loader.num_worker < self.cfg.data_loader.batch_size + 2
            else self.cfg.data_loader.num_worker
        )
        ...
        self.train_loader = DataLoaderX(
            ...
            shuffle=True
            if self.train_sampler is None and not self.cfg.data_loader.semi_dataset
            else False,
            drop_last=False,
            pin_memory=True,
    ...
    def _build_metric(self):
        self.train_metric_func = get_metric(
            metric=self.cfg.slover.metric,
            activation=self.cfg.model.activation,
            reduction="sum",
        )
        self.val_metric_func = get_metric(
            metric=self.cfg.slover.metric,
            activation=self.cfg.model.activation,
            reduction="sum",
        )
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.total_epoch + 1):
            ...
            if self.is_main_process:
                self.logger.info(
                    f"End of epoch {epoch}, time: {get_time(self.is_cuda)-epoch_start_time}"
                )
        ...
        if self.is_main_process:
            ...
            self.logger.info(
                f"Training time: {(run_end_time-run_start_time) / 60 / 60} hours"
            )
    ...
    def run_refine(self):
        ...
        for epoch in range(
            self.total_epoch, self.total_epoch + self.cfg.model.refine_epoch
        ):
            if self.is_main_process:
        ...
        for index, (images, masks) in enumerate(self.train_loader):
            ...
            if self.is_main_process:
                if index > 0 and index % self.cfg.slover.save_frequency == 0:
                    ...
                    self.logger.info(
                        "SegLoss:{:.6f}, LearnRate:{:.6f}".format(
                            seg_loss.item(), current_lr
                        )
                    )
    ...
    def _load_checkpoint(self):
        if (
            ...
            and self.pretrain_model_path is not None
    ...
    def _save_checkpoint(self, epoch, dice):
        ...
        if (
            self.is_main_process
            and self.cfg.slover.save_checkpoints
            and epoch
            in [
                int(self.total_epoch * 0.36),
                int(self.total_epoch * 0.66),
                int(self.total_epoch * 0.86),
                self.total_epoch,
            ]
        ):
            save_weight_path = os.path.join(
                self.model_dir, f"epoch_{str(epoch)}_model.pt"
            )
            torch.save(
                {
                    "lr": self.lr,
